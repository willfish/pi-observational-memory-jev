...
function isValidCoverageEntry(
	entry: Entry,
	customType: typeof OM_OBSERVATIONS_RECORDED | typeof OM_OBSERVATIONS_DROPPED,
	...
	return Array.isArray(entry.data.observationTimestamps) && entry.data.observationTimestamps.length > 0;
}
...
/**
 * Next observation chunk: source entries after `afterEntryId` until adding the next
 * valid cut-point would exceed `chunkTokens`. Always includes at least one source entry.
 * Never ends between a tool call and its result.
 */
